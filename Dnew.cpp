...
//    }
    ...
    bigint& operator += (const bigint& other) {
        ...
        for (uint64_t i = 0; i < std::max(other.numbers.size(), numbers.size()) + 1; ++i) {
...
public:
    ...
    void RemoveZeroes() {
        while (!monomials.empty() && monomials.back() == 0) {
            monomials.pop_back();
        }
        while (!module.empty() && module.back() == 0) {
            module.pop_back();
        }
    }
    ...
    void Remainder() {
        if (monomials.empty() || monomials.size() < module.size()) {
            return;
        }
        uint64_t degree = module.size() - 1;
...
int main() {
    ...
    while (std::getline(std::cin, rs)) {
        ...
        auto cipher = (m * s);
        answer.insert(answer.end(), cipher.Monomials().begin(), cipher.Monomials().end());
    }
